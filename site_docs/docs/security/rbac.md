...
## Authorization model: RBAC first, then policy { #authorization-model-rbac-first-then-policy }
...
## All roles (complete list) { #all-roles-complete-list }
...
### End-user roles (daily use) { #end-user-roles-daily-use }
...
### Read-only and special roles { #read-only-and-special-roles }
...
### Machine identity roles (automation / integration) { #machine-identity-roles-automation--integration }
...
## Role reference (summary) { #role-reference-summary }
