---
title: Compliance at stellarbridge
---
...
### Technology stack (what we build with) { #technology-stack-what-we-build-with }
...
### Build and containerization model (attack surface reduction) { #build-and-containerization-model-attack-surface-reduction }
...
### Runtime: Kubernetes (how we run it securely) { #runtime-kubernetes-how-we-run-it-securely }
...
### Identity, sessions, and authorization (what the app enforces) { #identity-sessions-and-authorization-what-the-app-enforces }
...
* Authorization
  * Role-based access control (RBAC) is enforced for API routes. Default roles are seeded; admins assign roles to users.

For more details, see [Security at stellarbridge](/docs/about-stellarbridge/security-at-stellarbridge/).

***

### Data flow: uploads and streaming (how files move) { #data-flow-uploads-and-streaming-how-files-move }
...
### Logging, audit, and abuse protection { #logging-audit-and-abuse-protection }
...
### Operational responsibilities (shared model) { #operational-responsibilities-shared-model }
