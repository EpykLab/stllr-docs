...
## Installing the server into your agent { #installing-the-server-into-your-agent }
...
### Other MCP clients (stdio) { #other-mcp-clients-stdio }
...
### `stellarbridge-mcp` not found after install { #stellarbridge-mcp-not-found-after-install }
...
### Wrong API or empty results { #wrong-api-or-empty-results }
...
### Agent does not see the tools { #agent-does-not-see-the-tools }
