...
# Writing Access Policies { #writing-access-policies }
...
### Migrating from legacy action names { #migrating-from-legacy-action-names }
...
### 1. Allow all for everyone { #1-allow-all-for-everyone }
...
### Secure-view sharing without download sharing { #secure-view-sharing-without-download-sharing }
...
### 2. Allow downloads only for a specific user { #2-allow-downloads-only-for-a-specific-user }
...
### 3. Allow list and download for a group { #3-allow-list-and-download-for-a-group }
...
### 4. Deny delete for everyone { #4-deny-delete-for-everyone }
...
### 5. Allow uploads for a specific API key { #5-allow-uploads-for-a-specific-api-key }
...
### 6. Allow a specific user but deny a specific API key { #6-allow-a-specific-user-but-deny-a-specific-api-key }
...
### 7. Multiple users with full access { #7-multiple-users-with-full-access }
...
### 8. View-only for external partners { #8-view-only-for-external-partners }
...
### 9. Editors can modify, others read-only { #9-editors-can-modify-others-read-only }
...
### 10. Block share links for confidential data { #10-block-share-links-for-confidential-data }
...
### 11. Risk approvers download only { #11-risk-approvers-download-only }
...
### 12. Admin-only policy management { #12-admin-only-policy-management }
...
### 13. Lock and freeze restricted to custodians { #13-lock-and-freeze-restricted-to-custodians }
...
### 14. Deny move and rename for shared folder { #14-deny-move-and-rename-for-shared-folder }
...
### 15. Allow copy but not delete { #15-allow-copy-but-not-delete }
...
### 16. A specific agent can only receive { #16-a-specific-agent-can-only-receive }
...
### 17. Audit-readers view-only with stream { #17-audit-readers-view-only-with-stream }
...
### 18. Contractors limited upload and list { #18-contractors-limited-upload-and-list }
...
### 19. Finance team full access, others denied { #19-finance-team-full-access-others-denied }
...
### 20. Read-only audit trail folder { #20-read-only-audit-trail-folder }
...
### 21. Mixed: specific user and a group { #21-mixed-specific-user-and-a-group }
...
### 22. Revoke share links only for admins { #22-revoke-share-links-only-for-admins }
...
### 23. JSON format example { #23-json-format-example }
